...
public class RemoveUser extends Command
{
    ...
    public void execute(CommandContext ctx)
    {
        ...
        ctx.getChannel().sendMessage(NoxLib.getInstance().getMessageUtil().createEmbedWithThumbnail("User Added to Ticket", "Removed " + ctx.getMessage().getMentionedMembers().get(0).getAsMention() + " from the ticket.", NoxLib.getInstance().getToolImage()).build()).queue();
    }
}
