...
public class AddUser extends Command
{
    ...
    public void execute(CommandContext ctx)
    {
        ...
        {
            ctx.getChannel().sendMessage(NoxLib.getInstance().getMessageUtil().createEmbedWithThumbnail("Incorrect Usage!", "Usage: " + this.getDescription(), NoxLib.getInstance().getImageDatabase().getErrorImage()).build()).queue(m -> m.delete().queueAfter(5, TimeUnit.SECONDS));
            return;
        }
        ...
        ctx.getChannel().sendMessage(NoxLib.getInstance().getMessageUtil().createEmbedWithThumbnail("User Added to Ticket", "Added " + ctx.getMessage().getMentionedMembers().get(0).getAsMention() + " to the ticket.", NoxLib.getInstance().getImageDatabase().getToolImage()).build()).queue();
