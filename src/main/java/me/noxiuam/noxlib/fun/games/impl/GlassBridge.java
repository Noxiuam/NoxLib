...
public class GlassBridge extends Game
{
    ...
    {
        ctx.getMessage().replyEmbeds(msg.createEmbedWithAuthor(this.getName(), "You have two pieces of glass in front of you.\n\nYou must jump through the rest as quickly as possible.\n\n" + this.gameBoard, "Levels Left: " + this.stagesLeft).build()).queue(m ->
        {
            m.addReaction("\u2B05").queue();
            m.addReaction("\u27A1").queue();
            this.message = m;
    ...
    public void handleGameInput(String choice)
    {
        ...
        if (willBreak)
        {
            ...
            this.message.editMessageEmbeds(NoxLib.getInstance().getMessageUtil().createEmbedWithAuthor(this.getName(), "You have jumped to the tile to the " + (movingRight ? "right" : "left") + ", but it broke and you fell to your death.\n\n" + this.gameBoard, "GAME OVER (Levels Left: " + this.stagesLeft + ")").build()).queue(m -> {
        ...
        else
        {
            if (this.stagesLeft == 0)
            {
                ...
                this.message.editMessageEmbeds(NoxLib.getInstance().getMessageUtil().createEmbed(this.getName(), "You have made it to the end!\n\nThank you for playing.").build()).queue(m -> m.delete().queueAfter(10, TimeUnit.SECONDS));
                this.endGame(this.memberId);
                return;
            }

            this.gameBoard = this.glassEmote + " " + this.glassEmote;
            this.message.editMessageEmbeds(NoxLib.getInstance().getMessageUtil().createEmbedWithAuthor("Please wait for the next level...", "You have jumped to the tile to the " + (movingRight ? "right" : "left") + ", and luckily, you lived.\n\n" + this.gameBoard, "Levels Left: " + this.stagesLeft).build()).queue(m -> m.editMessageEmbeds(NoxLib.getInstance().getMessageUtil().createEmbedWithAuthor(this.getName(), "You have two pieces of glass in front of you.\n\nYou must jump through the rest as quickly as possible.\n\n" + this.gameBoard, "Levels Left: " + this.stagesLeft).build()).queueAfter(3, TimeUnit.SECONDS));
        }
    }
}
