...
public class GameListener extends ListenerAdapter {
    ...
    public void onMessageReactionAdd(@NotNull MessageReactionAddEvent event) {
        if (!NoxLib.getInstance().getGameFramework().getPlayerList().containsKey(event.getMember()) || !NoxLib.getInstance().getGameFramework().getGamePieces().contains(event.getReactionEmote().getAsReactionCode()))
            return;
